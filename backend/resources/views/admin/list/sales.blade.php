        ...
        <div class="uk-modal-dialog">
            ...
            <form method="POST" action="{{ route('admin.edit.item.sell.stop') }}">
                ...
                <div class="uk-modal-body">
                    <div class="user__content__box__gray">
                        <p class="sub-headline">担当者名前</p>
                        <select name="charge" class="uk-select user__content__box__gray__inner" id="form-horizontal-select">
                            @foreach($charges as $charge)
                            <option value="{{ $charge->id }}">{{ $charge->name1 }} {{ $charge->name2 }}</option>
                            @endforeach
                        </select>
            ...
            <tbody>
                @foreach($sellItems as $sellItem)
                    <tr>
                        ...
                        <td>{{ $sellItem->branch_name }}<br>{{ $sellItem->charge_name1 }} {{ $sellItem->charge_name2 }}</td>
                        <td>
