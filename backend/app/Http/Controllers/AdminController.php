...
class AdminController extends Controller
{
    ...
    public function withdrawalRequest()
    {
        ...
        $charges = $chargeInstance->indexOfManager($managerId);
        // dd($charges);

        return view('admin.request.withdrawal', compact('withdrawals', 'charges'));
    }
    ...
    public function returnRequest()
    {
        ...
        return view('admin.request.return', compact('itemRequests', 'charges'));
    }
    ...
    public function returnRequestReject(Request $request)
    {
        ...
        try {
            ...
            // アイテムリクエスト非承認処理
            $itemRequest->decision_at = $today;
            $itemRequest->charge_id   = $request['charge'];
    ...
    public function salesRequest()
    {
        ...
        return view('admin.request.sales', compact('itemRequests', 'charges'));
    }
    ...
    public function salesRequestPost(Request $request)
    {
        ...
        try {
            ...
            $params['charge_id']        = $request['charge'];
    ...
    public function rentalRequest()
    {
        ...
        return view('admin.request.rental', compact('itemRequests', 'charges'));
    }
    ...
    public function rentalRequestPost(Request $request)
    {
        ...
        if($request['img2']) {
            $rulus[] = [
                'img2' => 'image|mimes:jpeg,png,jpg,gif|max:1024',
            ];
        }
        if($request['img3']) {
            $rulus[] = [
                'img3' => 'image|mimes:jpeg,png,jpg,gif|max:1024',
            ];
        }
        if($request['img4']) {
            $rulus[] = [
                'img4' => 'image|mimes:jpeg,png,jpg,gif|max:1024',
            ];
        }
        if($request['img5']) {
            $rulus[] = [
                'img5' => 'image|mimes:jpeg,png,jpg,gif|max:1024',
            ];
        }
        $message = [
    ...
    public function salesStopRequest()
    {
        ...
        $chargeInstance = new Charge;
        $charges = $chargeInstance->indexOfManager($managerId);
        return view('admin.request.stopSales', compact('itemRequests', 'charges'));
    }
    ...
    public function rentalStopRequest()
    {
        ...
        $chargeInstance = new Charge;
        $charges = $chargeInstance->indexOfManager($managerId);
        return view('admin.request.stopRental', compact('itemRequests', 'charges'));
    }
    ...
    public function rentalStopRequestPost(PermitItemRequest $request)
    {
        ...
        try {
            ...
            $itemRequest->charge_id   = $validated['charge']; // 担当者
    ...
    public function userList()
    {
        ...
        $chargeInstance = new Charge;
        $charges = $chargeInstance->indexOfManager($managerId);
        return view('admin.list.user', compact('users', 'charges'));
    }
    ...
    public function branchList()
    {
        ...
        // 支店一覧を取得
        $branches = Branch::where('manager_id', $managerId)->get();
        return view('admin.list.branch', compact('branches'));
    }
    ...
    public function branchAdd()
    {
        // 担当者と紐づく管理者情報
        $managerId = Auth::guard('manager')->user()->id;
        return view('admin.add.branch', compact('managerId'));
    }
    ...
    public function branchAddPost(Request $request)
    {
        ...
        $branchInstance->branchAdd($requests);
        // 担当者と紐づく管理者情報
        return redirect(route('admin.list.branch'))->with('success-message', '支店を追加しました。');
    }
     ...
     * @return view
     */
    public function branchEditStop(Request $request)
    {
        $branch = Branch::where('id', $request['target_id'])->first();
        $branch->delete();
        return redirect(route('admin.list.branch'))->with('success-message', '支店の削除に成功しました。');
    }
    ...
    {
        $managerId = Auth::guard('manager')->user()->id;
        $charges = Charge::where('manager_id', $managerId)->get();
        return view('admin.list.charge', compact('charges'));
    }
    ...
    public function chargeAdd()
    {
        // 担当者と紐づく管理者情報
        $managerId = Auth::guard('manager')->user()->id;
        return view('admin.add.charge', compact('managerId'));
    }
     ...
     * @return view
     */
    public function chargeAddPost(Request $request)
    {
        $requests = $request->all();
        $chargeInstance = new Charge;
        $chargeInstance->chargeAdd($requests);
        return redirect(route('admin.list.charge'))->with('success-message', '担当者を追加しました。');
    }
     ...
     * @return view
     */
    public function chargeEditStop(Request $request)
    {
        $charge = Charge::where('id', $request['target_id'])->first();
        $charge->delete();
    ...
    public function boxShow($id)
    {
        ...
        $box = $boxInstance->boxShowWithUserItem($id);
        $items = $itemInstance->itemWithJapanese($box->id);

        // 支店一覧を取得
        $managerId = Auth::guard('manager')->user()->id;
        $branches = Branch::where('branches.manager_id', $managerId)->get();
        ...
        return view('admin.show.box', compact('box', 'items', 'charges', 'rentals', 'branches'));
    }
    ...
    {
        $users = User::get();
        return view('admin.add.box', compact('users'));
    }
    ...
    public function itemList()
    {
        // 荷物一覧
        $itemInstance = new Item;
        $items = $itemInstance->itemWithJapaneseAll();
        ...
        // レンタルユーザー一覧を取得
        $rentals = RentalUser::all();
        return view('admin.list.item', compact('items', 'charges', 'rentals', 'branches'));
    }
    ...
    public function itemShow($id)
    {
        ...
        return view('admin.show.item', compact('item', 'itemImages', 'charges', 'branches'));
    }
     ...
     * @return view
     */
    public function itemAddPost(StoreItemRequest $request)
    {
        // バリデーションの値を取得
        $validated = $request->validated();
        ...
        try {
            // 荷物追加処理
            $itemInstance->itemStore($validated);
            $lastID = DB::getPdo()->lastInsertId();
            // 荷物画像 追加処理(item_images)
            $itemImageInstance->itemImageStore($validated, $lastID);
    ...
    // public function itemEditSold(Request $request)
    {
        ...
        try {
            // 売却情報(sales) 登録
            $saleParams['charge_id']    = $validated['charge_id'];
    ...
    public function itemEditRental(RentalItemRequest $request)
    {
        ...
        try {
            ...
            $params['charge_id']      = $validated['charge_id'];
    ...
    public function itemEditReturn(RentalReturnItemRequest $request)
    {
        ...
        $rental = $rentalInstance->rentalShowOfItem($item->id);
        // レンタル履歴情報取得
        $rentalTrade = $rentalTradeInstance->rentalTradeShowOfRental($rental->id);
    ...
    public function salesItemList()
    {
        ...
        return view('admin.list.sales', compact('sellItems', 'charges'));
    }
    ...
    public function rentalItemList()
    {
        ...
        return view('admin.list.rental', compact('managerId', 'rentalItems', 'charges', 'rentals'));
    }
